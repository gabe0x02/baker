 ...
 */
function handleHelp(command, filename) {
  if (!command) {
    // no command but help ... list commands
    printUsage(filename);
    printCommands();
  } else {
    printUsage(filename, command);
  }
}
...
function printUsage(filename, command) {
  if (!command) {
    console.error("Usage:", path.basename(filename), "COMMAND <options>");
  } else {
    console.error(command.getUsage(filename));
  }
}
